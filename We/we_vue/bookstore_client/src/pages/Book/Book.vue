      ...
      <div class="book_buy">
        ...
        <div class="book_content">
          ...
          <el-button type="primary" icon="el-icon-shopping-cart-2" class="cartBtn" @click="addBookToCart">加入购物车</el-button>
           <el-button class="applyBtn" @click="addAppication" >补单申请</el-button>
        </div>
      </div>
    </div>
    <div class="box">
...
<script>
    ...
    import {reqGetBookSortById, reqGetSortList} from "../../api/sort";
    import {reqAddCart} from "../../api/cart";
    import {reqAddSupply} from "../../api/supply"
    ...
    export default {
        ...
        methods: {
            ...
            //提交 补货申请
            addAppication() {
                reqAddSupply(this.$store.getters.getUser.account,this.book.id).then
                (
                  response=>{
                    if(response.code==200){
                        this.$message.success("已提交补单申请，商品上架后将对您进行消息提醒!")
                    }
                    else{
                      this.$message.error("提交补单申请失败")
                    }
                  }
                )
            },
