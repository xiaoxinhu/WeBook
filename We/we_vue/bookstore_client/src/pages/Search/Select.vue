        ...
        <div class="book_content" v-for="book in bookList">
          ...
          <div class="book_content_info">
            ...
            <div>
              ...
              <el-button class="plainBtn" @click="goBuyPage(book.id)" plain>立即购买</el-button>
            </div>
          </div>
        </div>
...
<script>
    ...
    import {reqGetRecBookList,reqGetBookListBySort,reqGetBookListByName} from "../../api/book";
    ...
    export default {
        ...
        methods: {
            ...
            //得到图书列表
            getBookList(name,page,pageSize){
                reqGetBookListByName(name,page,pageSize).then(response=>{
            ...
            handleCurrentChange(val) {
             ...
             //添加图书到购物车
            addBookToCart(id){
                reqAddCart(this.$store.getters.getUser.account,id,1).then(response=>{
                    if(response.code==200){
                        this.$message({
        ...
        created() {
            ...
            this.getSortList();
            this.getBookList(this.name,1,10);
        },
        watch: {
            $route(to, from) {
                this.sortId = this.$route.query.id;
