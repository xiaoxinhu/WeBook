    ...
    <div class="table_content">
      <el-table
        ...
        <el-table-column label="库存" width="80" align="center">
          <template  slot-scope="scope">
            <el-input v-model="scope.row.stock" align="center" @blur="handleInputNumber(scope.row)"></el-input>
          </template>
<!-- 
          prop="stock"
          label="库存"
...
<script>
    ...
    import {reqGetSupplyList,reqModifyStock} from "../../../../api/supply";
    ...
    export default {
        ...
        methods: {
            ...
            GetSort(page,pageSize){
                this.loading=false;
                reqGetSupplyList(page,pageSize).then(response=>{
            ...
            handleInputNumber(row) {
              reqModifyStock(row.id,row.stock).then(response=>{
                if(response.code==200){
                  this.$message.success("修改库存成功，同时将发送补货通知给已定订阅者")
                  this.GetSort(1,5);
                  //this.$router.push("/admin/supplyList")
                }
                else{
                  this.$message.err("修改失败，请重新修改")
                }
              })
              // reqModifyStock(row.id,row.stock).then(response=>{
              //       console.log(response);
